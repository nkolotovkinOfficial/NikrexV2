...
class Service:
    ...
    def sendMessage(self, phone):
        ...
        # Create suffixes
        okay = f"{Fore.YELLOW}Сервис ({getDomain(url)}) >> Сообщение отправлено!{Fore.RESET}"
        error = f"{Fore.MAGENTA}Сервис ({getDomain(url)}) >> Не удалось отправить сообщение!{Fore.RESET}"
        ...
        try:
            ...
            print(f"{Fore.RED}[ВРЕМЯ СОЕДИНЕНИЯ ИСТЕКЛО] {error}")
        except requests.exceptions.ConnectionError:
            print(f"{Fore.RED}[ОШИБКА СОЕДИНЕНИЯ] {error}")
        except Exception as err:
            print(err)
        else:
            # Check status
            if r.status_code == 200:
                print(f"{Fore.GREEN}[УСПЕШНО] {okay}")
            elif r.status_code == 429:
                print(f"{Fore.RED}[СЛИШКОМ МНОГО ЗАПРОСОВ] {error}")
            else:
