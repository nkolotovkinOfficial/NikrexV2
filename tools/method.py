...
class AttackMethod:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        print(f"{Fore.MAGENTA}[!] {Fore.BLUE}Атака завершена!{Fore.RESET}")
    ...
    def __RunThreads(self):
        ...
        # Start flood threads
        with Spinner(
            label=f"{Fore.YELLOW}Запуск {self.threads_count} потоки{Fore.RESET}",
            total=100,
        ) as spinner:
    ...
    def Start(self):
        ...
        print(
            f"{Fore.MAGENTA}[?] {Fore.BLUE}Начиная атаку на {target} используя метод {self.name}.{Fore.RESET}\n"
        ...
        try:
            ...
            self.is_running = False
            print(
                f"\n{Fore.RED}[!] {Fore.MAGENTA}Ctrl + C обнаружен. Остановка {self.threads_count} потоки..{Fore.RESET}"
            )
